...
int main(void)
{
	...
	int min, sec;		// minutes and swconds for running time
	int time;		//running time in second only
	double mtime;		// time in second for one mile
	...
	msec = (int) mtime % S_PER_M;	// find remaining second
	printf("You ran %1.2f km (%1.2f miles) in %d min, %d sec.\n", distk, distm, min, sec);
	printf("That pace corresponds to running a mile in %d min, ", mmin);
	printf("%d sec.\nYour average speed is %1.2f mph.\n", msec, rate);
